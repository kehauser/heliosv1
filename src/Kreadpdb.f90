...
! --- local_variables
  ...
  CHARACTER(LEN=3),DIMENSION (:), ALLOCATABLE :: ca_resName
  CHARACTER(LEN=4),DIMENSION (:), ALLOCATABLE :: p_atom_name, ca_atom_name
  INTEGER,         DIMENSION (:), ALLOCATABLE :: p_atom_serial,ca_atom_serial
...
ALLOCATE(p_atom_name(natoms), resName(natoms), p_atom_serial(natoms), resN(natoms) )
     ...
     iatom=0
      DO WHILE (pdb_test .NE.'END   ')
          ...
          ENDIF
        READ (7,3) pdb_test
        IF ( ( (pdb_test .EQ.'ATOM  ') .OR. (pdb_test .EQ. 'HETATM') ) .AND. (pdb_test .NE. 'REMARK' ) ) THEN
          BACKSPACE (7)
          READ (7,2) pdb_test
            IF (TRIM(ADJUSTL(pdb_test)) .EQ. TRIM(ADJUSTL(helix_atom_names))) THEN !remove whitespace, is this ur atom?
              BACKSPACE(7)
              iatom=iatom+1
     ...
     iatom=0
      DO WHILE (pdb_test .NE.'END   ')
        ...
        IF ( ( (pdb_test .EQ.'ATOM  ') .OR. (pdb_test .EQ. 'HETATM') ) .AND. (pdb_test .NE. 'REMARK' ) ) THEN
          BACKSPACE (7)
          READ (7,2) pdb_test
            IF (TRIM(ADJUSTL(pdb_test)) .EQ. 'P' ) THEN
              BACKSPACE(7)
              iatom=iatom+1
...
!---try to catch structure problem: If missing nucleotides, assume p_atom_serial will be 1:n, n+1,m
    ...
    write(*,*) p_atom_serial(1)
    strandbreak=0
    DO iatom=katom,natoms
       count=katom+1
       tatom=katom+p_atom_serial(iatom)
