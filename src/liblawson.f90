...
!mute      write ( *, '(a)' ) '  Sequence of candidate solutions, X'
    end if
  else
    write ( unit, '(a)' ) ' '
    if ( mode == 1 ) then
...
!mute      write ( unit, '(a)' ) '  Sequence of candidate solutions, X'
    end if
  end if
!
...
!
  ...
  do kblock = 1, nblock
     ...
     if ( star ) then
!mute       write (*,fmt1(mode)) (head(mode),j,j = j1,j2)
     else
!mute       write (unit,fmt1(mode)) (head(mode),j,j = j1,j2)
     end if

     do i = 1,m
        if ( star ) then
           if ( blknam ) then
!PRINT ONLY THE SOLN 3 FOR THE SEQUENCE OF CANDIDATE SOLUTIONS HK
             !write (*,fmt2(mode)) i,' ',(a(i,j),j = 3,3 )!j1,j2)
             write(*,*) (a(i,j),j = 3,3)
           else
             !write (*,fmt2(mode)) i,names(i),(a(i,j),j = 3,3) ! j1,j2)
             write(*,*) (a(i,j),j = 3,3)
           end if
        else
           if ( blknam ) then
!mute             write (unit,fmt2(mode)) i,' ',(a(i,j),j = j1,j2)
           else
!mute             write (unit,fmt2(mode)) i,names(i),(a(i,j),j = j1,j2)
           end if
        end if
      end do
...
!mute1    write (unit,300)
  end if

  ysq = 0.0D+00

!JUST NSOL  do j = 0, nsol
     j=nsol
     if ( j /= 0 ) then
     ...
     if ( star ) then
!just rnorm       write (*,310) j,ynorm,rnorm,yl,rl
         WRITE(*,*) 'RNORM(nsol=3): ', RNORM
     else
!just rnorm       write (unit,310) j,ynorm,rnorm,yl,rl
         WRITE(*,*) 'RNORM(nsol=3): ', RNORM
     end if
...
!!warning    if ( ipass == 2 ) then
!!warning      write ( *, '(a)' ) ' '
!!warning      write ( *, '(a)' ) 'SVDRS - Warning of Subaccuracy! '
!!warning      write ( *, '(a)' ) '  Full accuracy was not attained in the singular'
!!warning      write ( *, '(a)' ) '  value decomposition of the bidiagonal matrix.'
!!warning    end if
